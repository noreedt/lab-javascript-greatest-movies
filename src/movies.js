...
function scoresAverage(moviesArray) {
  ...
  let average = moviesArray.reduce(
    (acc, val) => acc + (typeof val.score === "number" ? val.score : 0),
    0
  );
...
function dramaMoviesScore(moviesArray) {
  ...
  let average = dramaMovies.reduce(
    (acc, val) => acc + (typeof val.score === "number" ? val.score : 0),
    0
  );
...
function orderAlphabetically(moviesArray) {
  ...
  let orderedTitles = onlyTitles.sort();
  return orderedTitles.length > 20 ? orderedTitles.slice(0, 20) : orderedTitles;
}
